...
void HelloTriangleApplication::createInstace()
{
    if (enableValidationLayers && !checkValidationLayerSuppert())
    {
...
void HelloTriangleApplication::updateUniformBuffer()
{
    ...
    ubo.proj[1][1] *= -1; // GLM最初是为OpenGL设计的，它的裁剪坐标的Y是反转的
...
void HelloTriangleApplication::setupDebugMessager()
{
    ...
    if (vkCreateDebugUtilsMessengerEXT(instance, &createInfo, nullptr, &debugMessager) != VK_SUCCESS)
    {
...
bool HelloTriangleApplication::checkValidationLayerSuppert()
{
    uint32_t layerCount;
